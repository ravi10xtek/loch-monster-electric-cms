...
/**
 * Button placed via afterInput on the Posts.excerpt field.
 * Calls /api/posts/[id]/auto-link which walks the Lexical body and
 * wraps the first occurrence of each glossary term and service name
 * in a link node.
 */
export function AutoLinkButton(_props: any) {
  let id: string | number | undefined
  try {
    const docInfo = useDocumentInfo()
    id = (docInfo as any)?.id
  } catch {
    id = undefined
  }
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
        ...
        <span style={{ fontSize: '12px', color: 'var(--color-base-600, #555)' }}>
          Wraps the first occurrence of each glossary term and service name in a link below. Existing links are preserved.
        </span>
      </div>
      {message && (
        <p
